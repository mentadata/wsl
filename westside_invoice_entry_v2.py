    ...
    for row in df.itertuples():
        ...
        if dbrow is None:  ##Item NOT FOUND through VPN, SO CHECK USING DIRECT SCANID
            ...
            if scancode is not None:
                ...
                if dbrow is None: ##Item NOT FOUND through DIRECT SCANID CHECK, SO CHECK USING SUE'S FILE MAPPING
                    
                    cur = cursor.execute("SELECT pos_scancode FROM xx_sue_file_pos_scancodes_to_full_barcodes WHERE full_scan_code like ?",scancode_q)
                    dbrow = cur.fetchone()    
                    
                    if dbrow is not None:  ##Item FOUND through Sue's file
                ...
                elif dbrow is not None: ##Item FOUND through scancode
       ...
       ####CHECK IF ITEM IDENTIFIED IN POS END ####
        ...
        if dbrow is None:
            ...
            params = [str(vpn),str(InvoiceNumber),sNo,PoNumber,QtyOrder,caseOrder,caseCost,totalCost,unitCost,file,row.UPCNumber,row.PackUPC,scancode,row.GLCode,source,row.ProductDescription]
...
if __name__ == "__main__":
    ...
    fulldf = pd.read_csv(downloaded_file[0],encoding='cp1252',names=df_col_headers,header=0,dtype={"ProductNumber":"str"})
